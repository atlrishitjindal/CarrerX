    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6 max-w-5xl mx-auto">
      ...
      {myApps.length === 0 ? (
        ...
        <div className="space-y-4">
          {myApps.map((app) => {
            // Priority: Snapshot Data -> Lookup Job -> Fallback
            const job = jobs.find(j => j.id === app.jobId);
            const companyName = app.companyName || job?.company || "Unknown Company";
            const location = app.location || job?.location;
            const salary = app.salary || job?.salary;
            const description = job?.description || ""; // Description isn't snapshotted currently to save space
